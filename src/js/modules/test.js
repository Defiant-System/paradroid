...
let Test = {
	init(APP) {
		...
		setTimeout(() => APP.transfer.dispatch({ type: "render-circuit-board" }), 50);
		setTimeout(() => {
			let svg = APP.transfer.els.cbLeft.find("svg");
			APP.transfer.dispatch({ type: "mirror-schema", svg });
		}, 100);
		setTimeout(() => APP.transfer.els.cbLeft.find(`.toggler div:nth-child(8)`).trigger("click"), 150);
		setTimeout(() => APP.transfer.els.cbLeft.find(`.toggler div:nth-child(10)`).trigger("click"), 150);
		// setTimeout(() => APP.transfer.els.cbRight.find(`.toggler div:nth-child(8)`).trigger("click"), 100);
		return;
